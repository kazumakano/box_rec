...
def _try(param: dict[str, util.Param]) -> None:
    ...
    datamodule = DataModule.load(path.join(train.get_context().get_storage().storage_path, train.get_context().get_experiment_name()))
    trainer = pl.Trainer(
        logger=TensorBoardLogger(path.join(train.get_context().get_trial_dir(), "log/"), name=None, default_hp_metric=False),
...
def tune_params(param_list_file: str, box_dir: list[str], bot_conf_file: Optional[str] = None, gpu_ids: Optional[list[int]] = None, result_dir_name: Optional[str] = None) -> None:
    ...
    datamodule = DataModule(DataModule.unpack_param_list(param_list), box_dir)
...
if __name__ == "__main__":
    ...
    tune_params(args.param_list_file, args.box_dir, args.bot_conf_file, args.gpu_ids, args.result_dir_name)
